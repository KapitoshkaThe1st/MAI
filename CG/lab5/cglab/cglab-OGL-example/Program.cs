...
public abstract class CGLabDemoOGL : OGLApplicationTemplate<CGLabDemoOGL>
{
    ...
    [DisplayNumericProperty(new[] { 1.5, 1.5, 1.5 }, 0.01, "Ks", 0.0, 3.0)]
    public virtual DVector3 Ks { get; set; }
    [DisplayNumericProperty(new[] { 1.2, 1.2, 1.2 }, 0.01, "Kd", 0.0, 3.0)]
    public virtual DVector3 Kd { get; set; }
    [DisplayNumericProperty(6.0, 1.0, "K", 0.0, 100d)]
    ...
    DMatrix4 OrthoNormalized(double W, double H)
    {
        ...
        if (W > H)
        {
            return Ortho(-1.0 * AspectRatio, 1.0 * AspectRatio, -1.0, 1.0, 2.0, -2.0);
        }
        else
        {
            return Ortho(-1.0, 1.0, -1.0 / AspectRatio, 1.0 / AspectRatio, 2.0, -2.0);
        }
    }
