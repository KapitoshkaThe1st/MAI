...
private:
	...
	void remove(node_ptr, const T&);
	void print(std::ostream&, node_ptr, int);
	node_ptr find(node_ptr, const T&);
...
template<typename T>
Iterator<T> BinTree<T>::Find(const T &obj){
	return Iterator<T>(find(m_root, obj), this);
}

template<typename T>
void BinTree<T>::Remove(const T &obj) {
	remove(m_root, obj);
}
...
template<typename T>
void BinTree<T>::remove(node_ptr tree, const T &obj) {
...
template<typename T>
node<T>* BinTree<T>::find(node_ptr tree, const T &obj) {
