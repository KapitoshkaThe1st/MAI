...
public:
	...
	BinaryTree();
	~BinaryTree();
	void Insert(const std::shared_ptr<T>&);
	void Erase(const std::shared_ptr<T>&);
	std::shared_ptr<T> Find(const std::shared_ptr<T>&);
	void Print();
private:
	node<T> *m_root;
	node<T>* insert(node<T>*, node<T>*);
	node<T>* find(node<T>*, const std::shared_ptr<T>&);
	node<T>* remove(node<T>*, const std::shared_ptr<T>&);
	node<T>* unlink_min(node<T>*);
	void print(std::ostream&, node<T>*, int);
	node<T>* del(node<T>*);
